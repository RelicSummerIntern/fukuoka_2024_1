					...
					<form action="{{ route('recipe.index') }}" method="POST">
						...
						<div class="row no-gutters align-items-center">
										...
										@if (isset($ingredients) && count($ingredients) > 0)
											@foreach ($ingredients as $ingredient)
												<div class="form-check">
													<input class="form-check-input" type="checkbox" id="{{ $ingredient }}" name="selected_ingredients[]"
														value="{{ $ingredient }}">
							...
							<div class="col-12 col-md-6 d-flex flex-column align-items-center justify-content-center">
								<img src="{{ asset('images/icon_recipe.png') }}" alt="Cooking" class="img-fluid"
									style="width: 100vw; height: auto;">
								<div class="text-center mt-4" style="width: 100%;">
									<button type="submit" class="btn btn-warning w-100"
										style="font-weight: bold; color: white; font-size: 1.75rem; padding: 20px;">献立を見る!</button>
								</div>
								<div class="text-center mt-2" style="width: 100%;">
									<a href="{{ route('home') }}" class="btn btn-warning w-100"
										style="font-weight: bold; color: white; font-size: 1.75rem; padding: 20px;">戻る</a>
								</div>
							</div>
						</div>
					</form>
